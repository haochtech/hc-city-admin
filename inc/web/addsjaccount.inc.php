...
$GLOBALS['frames'] = $this->getMainMenu();
    ...
    if (!empty($id)) {
        $account = pdo_fetch("SELECT * FROM " . tablename('zhtc_account') . " WHERE weid = :weid AND storeid=:id ORDER BY id DESC", array(':weid' => $_W['uniacid'], ':id' => $id));
 ...
 if (checksubmit('submit')) {
        ...
        if (empty($users)) {
            ...
            if ($uid > 0) {
             ...
             //operator
                ...
                pdo_insert('users_permission', array(
                    ...
                    'url' => '',
                    'type' => 'zh_tcwq',
                   'permission' => 'zh_tcwq_menu_stores2'
                ));
                pdo_insert('zhtc_account', array(
                    'uid' => $uid,
                    'weid' => $_W['uniacid'],
                    'storeid' =>$id,
                ...
                message('用户增加成功！!', $this->createWebUrl('addsjaccount', array('id'=>$id), true));
            }
        }else {
            ...
            message('更新成功！', $this->createWebUrl('addsjaccount', array('id'=>$id), true));
        }
        message('操作用户失败，请稍候重试或联系网站管理员解决！');
   }
include $this->template('web/addsjaccount');