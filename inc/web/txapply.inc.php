...
// 
$account=pdo_get('zhtc_account',array('cityname'=>$_COOKIE['cityname'],'weid'=>$_W['uniacid']),'money');
$kdyj=$account['money'];

   //商户已提现金额
   $ytxcost=pdo_get('zhtc_yjtx', array('account_id'=>$_COOKIE['account_id'],'status'=>2,'uniacid'=>$_W['uniacid']), array('sum(tx_cost) as ytxcost'));
   //商户申请提现金额
  $stxcost=pdo_get('zhtc_yjtx', array('account_id'=>$_COOKIE['account_id'],'status'=>1,'uniacid'=>$_W['uniacid']), array('sum(tx_cost) as stxcost'));
   //可提现金额
