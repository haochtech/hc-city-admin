...
function getapi($url){
	...
	if($state==2 or $grouporder['group_id']==0){
		getapi("".$url."/app/index.php?i=".$grouporder['uniacid']."&c=entry&a=wxapp&do=InsertStoreWallet&m=zh_tcwq&group_id=".$grouporder['group_id']."&order_id=".$grouporder['id']);//分销
		getapi("".$url."/app/index.php?i=".$grouporder['uniacid']."&c=entry&a=wxapp&do=PtMessage&m=zh_tcwq&group_id=".$grouporder['group_id']);//模板消息
	}
	}			
	}
if($qgorder['state']==1){
		...
		pdo_update('zhtc_qggoods',array('surplus -='=>1),array('id'=>$qgorder['good_id']));
         ...
         getapi("".$url."/app/index.php?i=".$qgorder['uniacid']."&c=entry&a=wxapp&do=QgMessage&m=zh_tcwq&order_id=".$qgorder['id']);//模板消息
	...
	if($fxlog['state']==1){
		...
		getapi("".$url."/app/index.php?i=".$fxlog['uniacid']."&c=entry&a=wxapp&do=Fx&m=zh_tcwq&user_id=".$fxlog['user_id']."&money=".$fxlog['money']);//分销
	...
	if($usercoupons['pay_type']==1){
		...
		pdo_update('zhtc_store',array('wallet +='=>$coupon['money']),array('id'=>$coupon['store_id']));
         ...
         getapi("".$url."/app/index.php?i=".$store['uniacid']."&c=entry&a=wxapp&do=HdMessage&m=zh_tcwq&id=".$usercoupons['id']);//分销
	}
	if($dmorder['state']==1){
	...
	if($order['pay_state']==1){
		$res=pdo_update('zhtc_distribution',array('pay_state'=>2),array('code'=>$logno));
			if($res){
				$yjset=pdo_get('zhtc_yjset',array('uniacid'=>$order['uniacid']));
			if($yjset['type']==1){
			...
			getapi("".$url."/app/index.php?i=".$order['uniacid']."&c=entry&a=wxapp&do=Fx&m=zh_tcwq&user_id=".$order['user_id']."&money=".$order['money']);//分销
		}
	...
	if($hdorder['state']==1){
		...
		if($system['is_bm']==1){
			getapi("".$url."/app/index.php?i=".$hdorder['uniacid']."&c=entry&a=wxapp&do=HdMessage&m=zh_tcwq&id=".$hdorder['id']);//模板消息
			pdo_update('zhtc_joinlist',array('state'=>3),array('code'=>$logno));
		}else{
			pdo_update('zhtc_joinlist',array('state'=>2),array('code'=>$logno));
		}
		getapi("".$url."/app/index.php?i=".$hdorder['uniacid']."&c=entry&a=wxapp&do=Fx&m=zh_tcwq&user_id=".$hdorder['user_id']."&money=".$hdorder['money']);//分销
		getapi("".$url."/app/index.php?i=".$hdorder['uniacid']."&c=entry&a=wxapp&do=ActYj&m=zh_tcwq&act_id=".$hdorder['act_id']."&money=".$hdorder['money']);//城市佣金
